...
class A1Env(gym.Env):
    ...
    def __init__(self):
        ...
        # TROT PHASE CLOCK
        #
        # Mimics the sine wave from hardcoded trot:
        #   t increments each step
        #   phase = sin(t)
        #   FL+RR swing when phase > 0  (pair B)
        #   FR+RL swing when phase < 0  (pair A)
        #
        # sin + cos together give robot full phase info
        # Robot can sync its legs to this clock
        # =====================================================

        self.phase_t    = 0.0
        self.phase_dt   = 0.015   # same as hardcoded trot

        # =====================================================
        # REFERENCE MOTION
        #
        # Joint offsets from hardcoded trot code:
        #
        # ctrl indices (12 total):
        ...
        #
        # From hardcoded trot:
        #   FL thigh (idx 4): -0.35 * phase
        #   FL knee  (idx 5): -0.06 * phase
        #   RR thigh (idx 7): -0.15 * phase
        #   RR knee  (idx 8): -0.10 * phase
        #   FR thigh (idx 1): +0.35 * phase
        #   FR knee  (idx 2): +0.06 * phase
        #   RL thigh (idx10): +0.15 * phase
        #   RL knee  (idx11): +0.10 * phase
        # =====================================================
        ...
        # =====================================================
        # CURRICULUM
        # cmd_vel 0.10 -> 0.35 m/s
        # =====================================================

        self.cmd_vel      = 0.10
        self.cmd_vel_min  = 0.10
        self.cmd_vel_max  = 0.35
        ...
        # previous action   12
        # cmd velocity       1
        # phase sin          1  <- NEW clock signal
        # phase cos          1  <- NEW clock signal
        #
        # total = 51
        # =====================================================
    ...
    # =========================================================
    # REFERENCE MOTION
    # Compute target joints from hardcoded trot sine wave
    # =========================================================
    ...
    def _get_strict_trot_reward(self, contacts):
        ...
        # State A: FR+RL stance, FL+RR swing
        score_A = (fr + (1.0-fl) + (1.0-rr) + rl) / 4.0

        # State B: FL+RR stance, FR+RL swing
        ...
        if total > 10:
            ...
            symmetry_penalty = 2.0 * abs(ratio - 0.5)

        # wrong pattern penalties
        front_wrong   = fr*fl + (1-fr)*(1-fl)       # front same phase
        rear_wrong    = rr*rl + (1-rr)*(1-rl)       # rear same phase
        all_stance    = fr*fl*rr*rl                  # all on ground
        all_air       = (1-fr)*(1-fl)*(1-rr)*(1-rl) # all in air
    ...
    def _get_obs(self):
        ...
        # phase clock — tells robot where it is in trot cycle
    ...
    def reset(self, seed=None, options=None):
        ...
        # randomize starting phase so robot learns
        # to trot from any phase, not just phase=0
    ...
    def step(self, action):
        ...
        # advance phase clock every step
        ...
        #
        # Reward robot for matching the hardcoded trot
        # joint targets at current phase
        # This directly teaches the sine wave pattern
        # ===================================================
        ...
        # ----- GOLDEN RULE: no forward reward without stepping -----
        ...
        if feet_in_air >= 1.0:
            ...
            forward_reward = 0.0
            coast_penalty  = 3.0 * fwd_vel_clipped
        ...
        # stance push only when exactly 2 feet on ground
        ...
        if forward_vel > 0.05:
            (
                ...
                wrong_penalty,
                stuck_penalty
            ) = self._get_strict_trot_reward(foot_contacts)
        ...
        reward = (
            ...
            + height_stability
            + early_stable_bonus
            + ref_reward             # reference motion from hardcoded trot
            - oscillation_penalty
            - smoothness_penalty
            - slide_penalty
            - lateral_penalty
